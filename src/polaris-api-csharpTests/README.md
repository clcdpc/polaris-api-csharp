...
Executable mutating tests are grouped by API surface rather than combined into one broad cross-surface workflow. The title-list surface workflow creates uniquely named disposable title lists owned by the current test run, finds those lists again by exact generated name, registers cleanup immediately after each successful create, and deletes only those generated lists during `finally` cleanup. When `TestSettings:BibId` is configured, the same focused workflow can exercise title-list item add/copy/move/delete operations using only lists created by that workflow.

Account-credit tests are not executable unless the generated credit/deposit artifact can be uniquely identified from patron account rows and reversed with a reliable refund, void, or equivalent cleanup call. Until the client/model surface and fixture data make that cleanup path reliable, account-credit coverage remains an inconclusive scenario-dependent placeholder and must not create durable patron account rows.

Patron block and patron note tests remain durable mutations because there is no reliable cleanup path in the current client surface. Run them only against disposable patrons in safe environments. Hard-coded invalid or nonexistent IDs are not safety mechanisms for mutating operations; any live mutating scenario that needs item, record-set, account, hold, notification, or title-list data must use explicitly configured disposable fixtures and a documented cleanup strategy or remain inconclusive.
...
| Patron account/title lists | account get, title lists get | None by default for payment/refund/void/title-list mutations | focused title-list surface workflow with create/find/item operations/delete cleanup for generated lists | account-credit create/deposit remain placeholders until generated rows can be uniquely identified and reversed; payment/refund/void require disposable account fixtures |
