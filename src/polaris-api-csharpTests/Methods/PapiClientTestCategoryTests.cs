...
namespace Clc.Polaris.Api.Tests
{
    ...
    public class PapiClientTestCategoryTests
    {
        ...
        [TestMethod]
        public void LivePapiClientMethodTestsHaveExactlyOneIntegrationCategory()
        {
            var incorrectlyCategorized = GetPapiClientMethodTestMethods()
                .Select(method => new
                {
                    method.Name,
            ...
            Assert.IsFalse(
                ...
                $"Expected every live PAPI client method test method to have exactly one integration category ({string.Join(", ", IntegrationCategoryNames)}): {string.Join(", ", incorrectlyCategorized)}");
        }

        [TestMethod]
        public void PapiClientMethodTestsDoNotUseLegacyIntegrationCategories()
        {
            ...
            var methodsWithLegacyCategories = GetPapiClientMethodTestMethods()
                .Select(method => new
                {
                    method.Name,
            ...
            Assert.IsFalse(
                ...
                $"Expected no PAPI client method test method to use legacy raw integration category traits: {string.Join(", ", methodsWithLegacyCategories)}");
        }

        [TestMethod]
        public void StaffOverrideIntegrationTestsHaveProtectedIntegrationCategory()
        {
            var unprotectedMethods = PapiClientMethodTestSourceMethods()
                .Where(method => method.Value.Contains("IntegrationTestRequirements.RequireStaffOverrideAccount(PapiSettings)"))
                .Select(method => GetPapiClientMethodTestMethod(method.Key))
        ...
        {
            var missingDoNotParallelize = GetPapiClientMethodTestMethods()
        ...
        public void ReadOnlyIntegrationTestsAreParallelizedUnlessDocumentedException()
        {
            ...
            var unexpectedlyNonParallelized = GetPapiClientMethodTestMethods()
        ...
        private static IEnumerable<MethodInfo> GetPapiClientMethodTestMethods()
        {
            return typeof(IntegrationTestBase).Assembly
                .GetTypes()
                .Where(type => type.IsSubclassOf(typeof(IntegrationTestBase)))
                .SelectMany(type => type.GetMethods(BindingFlags.Instance | BindingFlags.Public))
                .Where(method => method.GetCustomAttributes<TestMethodAttribute>(inherit: false).Any());
        }

        private static MethodInfo GetPapiClientMethodTestMethod(string methodName)
        {
            var method = GetPapiClientMethodTestMethods()
                .SingleOrDefault(method => method.Name == methodName);

            Assert.IsNotNull(method, $"Expected to find public PAPI client method test method {methodName}.");
        ...
        private static void AssertMethodsHaveCategory(IEnumerable<string> methodNames, string expectedCategory)
        {
            var missingCategory = methodNames
                .Where(methodName => !MethodCategories(GetPapiClientMethodTestMethod(methodName)).Contains(expectedCategory))
                .ToArray();

            Assert.IsFalse(
                missingCategory.Any(),
                $"Expected these PAPI client method test methods to be marked with {expectedCategory}: {string.Join(", ", missingCategory)}");
        }
        ...
        private static Dictionary<string, string> PapiClientMethodTestSourceMethods([CallerFilePath] string categoryTestSourcePath = "")
        {
