...
namespace Clc.Polaris.Api
{
    public partial class PapiClient : RestClient, IPapiClient
    {
        ...
        private async Task<ProtectedToken?> EnsureProtectedTokenAsync(CancellationToken cancellationToken = default)
        {
            ...
            {
                return await AuthenticateAndLoadProtectedTokenAsync(cacheKey, staffOverrideAccount, cancellationToken).ConfigureAwait(false);
            }
        ...
        private async Task<ProtectedToken?> AuthenticateAndLoadProtectedTokenAsync(string? cacheKey, PolarisUser staffOverrideAccount, CancellationToken cancellationToken)
        {
            ...
            if (response?.Response == null ||
                ...
                (responseData == null || IsProtectedTokenMissingOrExpired(responseData)) ||
        ...
        private string GetPAPIHash(string httpMethod, string date, string uri, string password)
        {
            ...
            using var hmac = new HMACSHA1(Encoding.UTF8.GetBytes(AccessKey ?? string.Empty));
