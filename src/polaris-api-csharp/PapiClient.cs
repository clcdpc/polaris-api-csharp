...
namespace Clc.Polaris.Api
{
    public partial class PapiClient : RestClient, IPapiClient
    {
        ...
        {
            if (request.AuthRequired && request.IsPublicMethod && AllowStaffOverrideRequests && string.IsNullOrWhiteSpace(request.Password) && !request.BlockStaffOverride)
            {
        ...
        private void TryLoadCachedProtectedToken()
        {
        ...
        private async Task<ProtectedToken> GetTokenAsync(CancellationToken cancellationToken = default)
        {
            TryLoadCachedProtectedToken();
